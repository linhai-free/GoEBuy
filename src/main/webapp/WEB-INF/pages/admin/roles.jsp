...
<div class="container">
    ...
    <h3>所有角色<a href="/admin/roles/add" eventType="button" class="btn btn-primary btn-sm">添加</a></h3>

    <!-- 如果用户列表为空 -->
    <c:if test="${empty roleList}">
        <div class="alert alert-warning" role="alert">
            <span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span>Role表为空，请<a href="/admin/roles/add" eventType="button" class="btn btn-primary btn-sm">添加</a>
        </div>
    </c:if>
            ...
            <c:forEach items="${roleList}" var="role">
                <tr>
                    ...
                    <td>${role.role}</td>
                     <td>${role.desc}</td>
                    <td>
                        <a href="/admin/roles/show/${role.id}" eventType="button" class="btn btn-sm btn-success">详情</a>
                        <a href="/admin/roles/update/${role.id}" eventType="button" class="btn btn-sm btn-warning">修改</a>
                        <a href="/admin/roles/delete/${role.id}" eventType="button" class="btn btn-sm btn-danger">删除</a>
                    </td>
                </tr>
            </c:forEach>
        </table>
...
<ineventTypetype="fibankNamename="file"/>
<ineventTypetype="submit" value="上传文件"/>
